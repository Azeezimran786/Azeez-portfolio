...
let img = new Image()
img.src = "src/png/IMG_1667.jpg" // Make sure the image file is in the same directory
...
let cloth = new Cloth()
    ...
    ; (function update(time) {
        ...
        // 🖼 Draw image inside cloth area
        const clothWidth = clothX * spacing // 54 * 8 = 432
        const clothHeight = clothY * spacing // 28 * 8 = 224
        const startX = canvas.width / 2 - clothWidth / 2
        const startY = 20

        if (img.complete) {
            ctx.drawImage(img, startX, startY, clothWidth, clothHeight)
        }
